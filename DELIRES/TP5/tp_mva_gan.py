    ...
    def train(self, epochs, batch_size=128, sample_interval=50):
        ...
        for epoch in range(first_iter, epochs):
            ...
            for i in range(0, k):
                ...
                # START INSERT CODE
                d_loss_real = self.discriminator.train_on_batch(imgs, np.ones(batch_size))
                d_loss_fake = self.discriminator.train_on_batch(gen_imgs, np.zeros(batch_size))
                # END INSERT CODE
...
if __name__ == '__main__':
    ...
    if (is_training == 1):
        gan.train(epochs=10001, batch_size=64, sample_interval=500)
    else:
