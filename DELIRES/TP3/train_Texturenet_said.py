...
for parameter in gen.parameters():
    print(list(parameter.shape))
    s += np.prod(list(parameter.shape))
print(s)
assert False
############################################""
...
parser.add_argument("-i","--image",default='example.jpg', help="example image" ,type=str)
parser.add_argument("-m","--model_directory", default=None,help="directory where to save training", type=str)
parser.add_argument("--vgg_path",default=None,type=str,help="directory under which to find the vgg weights"  )
parser.add_argument("-s","--sample_generation",help="boolean True/False, default False",default=False,const=True,nargs='?',type=bool)
args=parser.parse_args()

# get descriptor network
# 'max' used in the paper
# 'avg' recommended
if args.vgg_path is None:
    vgg_path='./Models'
else:
    vgg_path=args.vgg_path
    
vgg = VGG(pool='max', pad=1)
vgg.load_state_dict(torch.load(vgg_path+'/vgg_conv.pth'))
...
input_base=os.path.splitext(input_base)
if args.model_directory is None:
    time_info = datetime.datetime.now()
    out_folder_name='./Trained_models/'+ time_info.strftime("%Y-%m-%d") + '_' \
                  + input_base \
                  + '_2D' + time_info.strftime("_%H%M")
else:
    out_folder_name=args.model_directory
...
# load images
input_texture = Image.open(input_name)
original_image_size=input_texture.size[0]
...
for n_iter in range(max_iter):
    ...
    if n_iter%save_params == (save_params-1):
        out_img = postp(sample.data.cpu().squeeze())
        out_img.save(out_folder_name + '/training_'
    ...
    if n_iter%save_params == (save_params-1):
        torch.save(gen,  out_folder_name
                   + '/trained_model_' + str(n_iter+1) + '.py')
        torch.save(gen.state_dict(),  out_folder_name
...
torch.save(gen,out_folder_name +'/trained_model.py')
torch.save(gen.state_dict(),out_folder_name+'/params.pytorch')
numpy.save(out_folder_name+'/loss_history',loss_history)

# sample after Training -------------------------------------------------------

if args.sample_generation:

    offline_size = original_image_size
    n_samples = 5
    ...
    for n in range(n_samples):
        ...
        out_img.save( out_folder_name + '/offline_sample_'
                     + str(n) + '.png')
